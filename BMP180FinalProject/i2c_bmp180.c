...
static int __init bmp180_init(void)
{
    ...
    deviceClass = class_create(DEVICE_CLASS);
    if ( deviceClass == NULL )
    {
...
static int bmp180_probe(struct i2c_client* client)
{
    ...
    if (ret < 0) {
        DBGMSG("|  |  i2c_master_send failed: %d\n", ret);
        return ret;
    }
    buf[0] = 0;
    ret = i2c_master_recv(client, buf, 1);
    if (ret < 0) {
        DBGMSG("|  |  i2c_master_recv failed: %d\n", ret);
        return ret;
    }
    DBGMSG("|  |  chip id read: 0x%02x, expected: 0x%02x\n", buf[0], CHIP_ID);
    if (buf[0] != CHIP_ID) {
        DBGMSG("|  |  error chip id: expected %d, received %d\n", CHIP_ID, buf[0]);
        DBGMSG("|  ▲ bmp180_probe failed\n");
        return -ENODEV;
...
static long int bmp180_ioctl(struct file* pfile, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        case IOCTL_BMP180_GET_TEMP:
            ...
            if (copy_to_user((long*)arg, &t, sizeof(long))) {
                DBGMSG("|  copy_to_user failed for IOCTL_BMP180_GET_TEMP\n");
                return -EFAULT;
            }
            break;
        ...
        case IOCTL_BMP180_GET_PRES:
            ...
            if (copy_to_user((long*)arg, &p, sizeof(long))) {
                DBGMSG("|  copy_to_user failed for IOCTL_BMP180_GET_PRES\n");
                return -EFAULT;
            }
            break;
        ...
        case IOCTL_BMP180_GET_ALTI:
            ...
            if (copy_to_user((long*)arg, &a, sizeof(long))) {
                DBGMSG("|  copy_to_user failed for IOCTL_BMP180_GET_ALTI\n");
                return -EFAULT;
            }
            break;
        ...
        case IOCTL_BMP180_ACTIVATE_VENTILATION:
            DBGMSG("|  cmd: IOCTL_BMP180_ACTIVATE_VENTILATION\n");
            // Placeholder: Add ventilation activation logic here
            break;

        case IOCTL_BMP180_DEACTIVATE_VENTILATION:
            DBGMSG("|  cmd: IOCTL_BMP180_DEACTIVATE_VENTILATION\n");
            // Placeholder: Add ventilation deactivation logic here
            break;

        case IOCTL_BMP180_SET_SUSPENSION:
            DBGMSG("|  cmd: IOCTL_BMP180_SET_SUSPENSION\n");
            // Placeholder: Add suspension adjustment logic here
            break;

        case IOCTL_BMP180_INCREASE_PRESSURE:
            DBGMSG("|  cmd: IOCTL_BMP180_INCREASE_PRESSURE\n");
            // Placeholder: Add pressure increase logic here
            break;

        case IOCTL_BMP180_DECREASE_PRESSURE:
            DBGMSG("|  cmd: IOCTL_BMP180_DECREASE_PRESSURE\n");
            // Placeholder: Add pressure decrease logic here
            break;

        case IOCTL_BMP180_GET_SPEED:
            DBGMSG("|  cmd: IOCTL_BMP180_GET_SPEED\n");
            // Placeholder: Add speed retrieval logic here
            break;

        case IOCTL_BMP180_ACTIVATE_COOLING:
            DBGMSG("|  cmd: IOCTL_BMP180_ACTIVATE_COOLING\n");
            // Placeholder: Add cooling activation logic here
            break;

        case IOCTL_BMP180_ACTIVATE_HEATING:
            DBGMSG("|  cmd: IOCTL_BMP180_ACTIVATE_HEATING\n");
            // Placeholder: Add heating activation logic here
            break;

        case IOCTL_BMP180_DEACTIVATE_HVAC:
            DBGMSG("|  cmd: IOCTL_BMP180_DEACTIVATE_HVAC\n");
            // Placeholder: Add HVAC deactivation logic here
            break;

        default:
            DBGMSG("|  Unknown IOCTL command: %u\n", cmd);
            return -EINVAL;
    }
